...
class TestRuns:
    ...
    def test_method_get_status(self, client: Anchorbrowser) -> None:
        run = client.tasks.runs.get_status(
            "runId",
        )
    ...
    def test_raw_response_get_status(self, client: Anchorbrowser) -> None:
        response = client.tasks.runs.with_raw_response.get_status(
            "runId",
        )
    ...
    def test_streaming_response_get_status(self, client: Anchorbrowser) -> None:
        with client.tasks.runs.with_streaming_response.get_status(
            "runId",
        ) as response:
    ...
    def test_path_params_get_status(self, client: Anchorbrowser) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `run_id` but received ''"):
            client.tasks.runs.with_raw_response.get_status(
                "",
            )
...
class TestAsyncRuns:
    ...
    async def test_method_get_status(self, async_client: AsyncAnchorbrowser) -> None:
        run = await async_client.tasks.runs.get_status(
            "runId",
        )
        assert_matches_type(RunGetStatusResponse, run, path=["response"])
    ...
    async def test_raw_response_get_status(self, async_client: AsyncAnchorbrowser) -> None:
        response = await async_client.tasks.runs.with_raw_response.get_status(
            "runId",
        )
    ...
    async def test_streaming_response_get_status(self, async_client: AsyncAnchorbrowser) -> None:
        async with async_client.tasks.runs.with_streaming_response.get_status(
            "runId",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
    ...
    async def test_path_params_get_status(self, async_client: AsyncAnchorbrowser) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `run_id` but received ''"):
            await async_client.tasks.runs.with_raw_response.get_status(
                "",
            )
